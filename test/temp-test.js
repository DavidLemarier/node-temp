...
temp.mkdir('foo', function (err, tpath) {
  ...
  assert.ok(
    path.basename(tpath).slice(0, 3) == 'foo',
    'temp.mkdir did not use the prefix'
  )
  ...
  assert.ok(
    !existsSync(tpath),
    'temp.cleanupSync did not remove the directory'
  )
...
temp.open('bar', function (err, info) {
  ...
  assert.equal(
    'object',
    typeof info,
    'temp.open did not invoke the callback with the err and info object'
  )
  assert.equal(
    'number',
    typeof info.fd,
    'temp.open did not invoke the callback with an fd'
  )
  ...
  assert.equal(
    'string',
    typeof info.path,
    'temp.open did not invoke the callback with a path'
  )
...
assert.ok(
  stream instanceof fs.WriteStream,
  'temp.createWriteStream did not invoke the callback with the err and stream object'
)
stream.write('foo')
...
assert.ok(
  !existsSync(stream.path),
  'temp.cleanupSync did not remove the createWriteStream file'
)
assert.ok(
  !existsSync(tempDir),
  'temp.cleanupSync did not remove the mkdirSync directory'
)
...
assert.ok(
  existsSync(tempFile.path),
  'temp.openSync did not create a file for cleanup'
)
...
temp.cleanup(function (err, counts) {
  ...
  assert.ok(
    !existsSync(tempFile.path),
    'temp.cleanup did not remove the openSync file for cleanup'
  )
  assert.equal(
    1,
    counts.files,
    'temp.cleanup did not report the correct removal statistics'
  )
})

var tempPath = temp.path()
assert.ok(
  path.dirname(tempPath) === temp.dir,
  'temp.path does not work in default os temporary directory'
)

tempPath = temp.path({ dir: process.cwd() })
assert.ok(
  path.dirname(tempPath) === process.cwd(),
  'temp.path does not work in user-provided temporary directory'
)
...
assert.equal(
  ...
  1,
  'temp created more than one listener for exit'
)
